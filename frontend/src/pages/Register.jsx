...
function Register() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // 2. Send data to the backend Register route
      const response = await fetch('http://localhost:5000/api/users/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          username: formData.name, // Mapping 'name' to 'username' used in your Backend Model
      ...
      if (response.ok) {
        ...
        // 3. Navigate to login page so they can sign in
        navigate('/login') 
      } else {
